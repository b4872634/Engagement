...
{
    public class AddEngagementController : Controller
    {
        private EngagementContext db = new EngagementContext();
        public ActionResult Index(string Encript)
        {
            int[] code = GetCode(Encript);
            int x = 2;
            x = code[0];
        ...
        public string GetUrl(int id)
        {
            ...
            var userUrl = from u in db.TB_Url where ((u.engagepersonal_ID == id))
                select u;
        ...
        public ActionResult AddInformationEmp(int page,string sid)
        {
            ...
            var engageid = code[1];
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInformationEmp", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
            }
            chkSection = chkInputData(engageid,1);//418 เอา id ไปหาว่าได้ตอบแบบสอบถามในส่วนที่ 1 ไปแล้วหรือไม่ 
            switch (x)
            {
                case 0://ยังไม่ได้ทำแบบทดสอบ 
                    break;
        ...
        public void GetDepartment()
        {
            ...
            ViewBag.departmentList = new SelectList(departmentList, "InstitutionUnit_Department_ID", "InstitutionUnit_Department_TH");
        }
        public JsonResult GetDivision(int id)
        {
            ...
            listDivision.Add(new SelectListItem { Text = "--Select สังกัด--", Value = "0" });
            if (ddlDivision != null)
            {
                foreach (var x in ddlDivision)
                {
                    listDivision.Add(new SelectListItem { Text = x.InstitutionUnit_Division_TH, Value = x.InstitutionUnit_Division_ID.ToString() });
                }
            }
        ...
        public ActionResult AddInEngageFirst(int page,string sid)
        {
            ...
            var engageid = code[1];
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageFirst", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
        ...
        public ActionResult AddInEngageSecond(int page, string sid)
        {
            ...
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageSecond", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
...

        {
            ...
            var engageid = code[1];
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageThird", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
...

        {
            ...
            var engageid = code[1];
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageFour", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
...

        {
            ...
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageFive", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
...

        {
            ...
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageSix", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
...

        {
            ...
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageSeven", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
...

        {
            ...
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageEight", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
        ...
        public ActionResult AddInEngageNine(int page, string sid)
        {
            ...
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageNine", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
        ...
        public ActionResult AddInEngageTen(int page, string sid)
        {
            ...
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("AddInEngageTen", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
        ...
        public ActionResult EndofTest(int page, string sid)
        {
            ...
            var engageid = code[1];
            var currentUrl = string.Empty;
            var fullUrl = this.Url.Action("EndofTest", "AddEngagement", null, this.Request.Url.Scheme);
            currentUrl = GetUrl(engageid);
            if (currentUrl == string.Empty)
            {
                SaveCurrentUrl(fullUrl, engageid);
            ...
            else
            {
                //update url
                updateUrl(fullUrl, engageid);
                var userDt = DateTime.Now.ToString("MM/dd/yyyy");       // return 08/05/2016 12:56 PM   MM/dd/yyyy HH:mm:ss
                DateTime oDateT = DateTime.ParseExact(userDt, "MM/dd/yyyy", System.Globalization.CultureInfo.InvariantCulture);
                Int32 engagementid = engageid;
                var engagementData = db.TB_Email.Where(x => x.engagepersonal_ID == engagementid).FirstOrDefault();
                if (engagementData != null)
        ...
        public void GetChoice_topiccomment()
        {
            ...
            ViewBag.choiceListComment = new SelectList(choiceListComment, "Choice_ID", "ChoiceTextTH");
        }
    }
}